...
void vm_swap_init(void){
    ...
    swap_total_pages = block_size(swap_disk_partition) / SECTORS_PER_PAGE;
...
uint32_t vm_swap_flush_kpage_to_disk(void* kpage){
    ...
    if(starting_disk_sector_for_kpage > swap_total_pages){// BITMAP ERROR == 18446744073709551615UL
    ...
    for (i = 0; i < SECTORS_PER_PAGE; ++ i) {//1 kpage == 8 disk sectors
        block_write (swap_disk_partition, // device driver object
            starting_disk_sector_for_kpage * SECTORS_PER_PAGE + i, // disk sector number of 1 page
            kpage + (BLOCK_SECTOR_SIZE * i) // src: buffer/kpage 
        );
    }
    
    // 3. u() bitamp
    bitmap_set(free_swap_disk_pages, starting_disk_sector_for_kpage, false);
    lock_release (&swap_lock);

    return starting_disk_sector_for_kpage;
}
